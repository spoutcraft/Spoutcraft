...
public class PacketWidget implements SpoutPacket {
	...
	public void run(int playerId) {
		InGameScreen mainScreen = Spout.getMainScreen();
		...
		if (widget instanceof PopupScreen) {
			if (popup != null){
				if (widget.getId().equals(popup)) {
					if (Spout.getGameInstance().currentScreen instanceof CustomScreen) {
						((CustomScreen)Spout.getGameInstance().currentScreen).update((PopupScreen)widget);
					}
				}
			}
			else {
