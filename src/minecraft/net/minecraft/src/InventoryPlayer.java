...
public class InventoryPlayer implements IInventory {
	...
	/**
	 * Set true whenever the inventory changes. Nothing sets it false so you will have to write your own code to check it
	 * and reset the value.
	 */
		...
		for (int var4 = 0; var4 < var3; ++var4) {
			...
			if (var5 != null) {
				if( var5.getItem() instanceof ItemArmor ) {
					...
					var1 += var6;
				} else if(var5.itemID == 318) {
					org.spoutcraft.spoutcraftapi.material.CustomItem item 
						= MaterialData.getCustomItem(var5.getItemDamage());
					if( item != null && item instanceof GenericCustomArmor ) {
						var1 += ((GenericCustomArmor)item).getDefense();
					}
				}
			}
			//Spout end
	...
	public void damageArmor(int par1) {
		...
		for (int var2 = 0; var2 < this.armorInventory.length; ++var2) {
			if (this.armorInventory[var2] != null) {
				//Spout start
				if( !(this.armorInventory[var2].getItem() instanceof ItemArmor) )
					continue;
				else if( this.armorInventory[var2].itemID == 318 ) {
					org.spoutcraft.spoutcraftapi.material.CustomItem item = MaterialData.getCustomItem(
								armorInventory[var2].getItemDamage());
					if( item == null || !(item instanceof GenericCustomArmor ) ) {
						continue;
					}
				}
