	...
	public void onUpdate() {
		...
		if(this.worldObj.multiplayerWorld) {
			if(this.field_9394_d > 0) {
				var21 = this.posX + (this.boatX - this.posX) / (double)this.field_9394_d;
				var6 = this.posY + (this.boatY - this.posY) / (double)this.field_9394_d;
				var8 = this.posZ + (this.boatZ - this.posZ) / (double)this.field_9394_d;
			...
			if(this.onGround && !landBoats) { //Spout start
				this.motionX *= 0.5D;
				this.motionY *= 0.5D;
				this.motionZ *= 0.5D;
			}
