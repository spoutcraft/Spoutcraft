...
public class SlotArmor extends Slot {
	...
	/**
	 * 35 Check if the stack is a valid item for this slot. Always true beside
	 * for the armor slots.
	 */
	public boolean isItemValid(ItemStack par1ItemStack) {
		// Spout start
		if (par1ItemStack.itemID == 318) {
			org.spoutcraft.spoutcraftapi.material.CustomItem item = MaterialData
					.getCustomItem(par1ItemStack.getItemDamage());
			if (item != null) {
				if (item instanceof GenericCustomArmor)
					return (((GenericCustomArmor) item).getType() == this.armorType);
				return false;
			}
		}
		// Spout end
		return par1ItemStack.getItem() instanceof ItemArmor ? ((ItemArmor) par1ItemStack
				.getItem()).armorType == this.armorType
				: (par1ItemStack.getItem().shiftedIndex == Block.pumpkin.blockID ? this.armorType == 0
						: false);
	}
