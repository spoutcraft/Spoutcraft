...
public class ChatManager {
	...
	public int getChatHistorySize() {
		return Spout.getGameInstance().ingameGUI.chatMessageList.size();
	}
	...
	public void sendChat(String message) {
		...
		for (String chat : lines) {
			Spout.getGameInstance().thePlayer.sendChatMessage(chat);
		}
	}
			...
			if (command.startsWith("~bind")) {
				...
				if (key >= 'A' && key <= 'Z') {
					...
					if (command.startsWith("/")) {
						...
						Spout.getGameInstance().ingameGUI.addChatMessage(ChatColor.GREEN.toString() + "Successfully bound key '" + key + "' to the command '" + command + "'");
						return true;
					}
				}
			}
				...
				if (command.length() == 1) {
					...
					unbind(key);
					Spout.getGameInstance().ingameGUI.addChatMessage(ChatColor.GREEN.toString() + "Successfully unbound key '" + key + "'");
					return true;
				}
			}
