...
public class PacketSky implements SpoutPacket{
	...
	public void run(int PlayerId) {
		if (cloudY != 0) {
			Spout.getSkyManager().setCloudHeight(cloudY);
		}
		if (stars != 0) {
			Spout.getSkyManager().setStarFrequency(stars);
		}
		if (sunPercent != 0) {
			Spout.getSkyManager().setSunSizePercent(sunPercent);
		}
		if (moonPercent != 0) {
			Spout.getSkyManager().setMoonSizePercent(moonPercent);
		}
		if (sun != null) {
			if (sun.equals("[reset]")) {
				Spout.getSkyManager().setSunTextureUrl(null);
			}
			else if (sun.length() > 5) {
				Spout.getSkyManager().setSunTextureUrl(sun);
			}
		}
		if (moon != null) {
			if (moon.equals("[reset]")) {
				Spout.getSkyManager().setMoonTextureUrl(null);
			}
