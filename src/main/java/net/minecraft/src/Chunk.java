...
public class Chunk {
	...
	/**
	 * Accepts a 256-entry array that contains a 16x16 mapping on the X/Z plane of block positions in this Chunk to biome
	 * IDs.
	 */
	...
	/**
	 * Called once-per-chunk-per-tick, and advances the round-robin relight check index per-storage-block by up to 8 blocks
	 * at a time. In a worst-case scenario, can potentially take up to 1.6 seconds, calculated via (4096/(8*16))/20, to re-
	 * check all blocks in a chunk, which could explain both lagging light updates in certain cases as well as Nether
	 * relight
	 */
