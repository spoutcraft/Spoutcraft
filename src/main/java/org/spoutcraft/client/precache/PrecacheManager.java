	...
	public static void loadPrecache(boolean reloadRenderer) {
		...
		for (Entry entry : plugins.entrySet()) {
			try {
				...
				final ZipFile zip = new ZipFile(getPluginPreCacheFile(plugin));
				final Enumeration zipEntries = zip.entries();
				...
				while (zipEntries.hasMoreElements()) {
					...
					final File target = new File(cacheRoot, plugin.getPlugin() + "/" + zipEntry.getName());
					if (!isCached) {
						if (target.exists()) {
							target.delete();
						}
						...
						final BufferedInputStream is = new BufferedInputStream(zip.getInputStream(zipEntry));
						int currentByte;
						final int BUFFER_SIZE = 100 * 1024;
						final byte[] buf = new byte[BUFFER_SIZE];

						final BufferedOutputStream dest = new BufferedOutputStream(new FileOutputStream(target), BUFFER_SIZE);

						while ((currentByte = is.read(buf, 0, BUFFER_SIZE)) != -1) {
					...
					if (target.exists()) {
						if (target.getName().endsWith(".sbd")) {
							loadDesign(target);
						} else if (FileUtil.isImageFile(target.getName())) {
