...
import com.pclewis.mcpatcher.mod.BiomeHelper$New;
...
public class Colorizer {
	...
	public static int colorizeBiomeWithBlending(int var0, int var1, int var2, int var3, int var4) {
		return colorizeWithBlending(fixedColorMaps[var1], var0, var2, var3, var4);
	}
	...
	private static int colorizeWithBlending(ColorMap var0, int var1, int var2, int var3, int var4) {
		if (var0 != null && var0.isCustom() && BiomeHelper.instance.useBlockBlending() && blockBlendRadius > 0) {
		...
		if (var1 != null && useLightmaps) {
			...
			if (var4 == null) {
				...
				if (var6 != 32) {
					...
					float var9 = clamp(var1.lightningFlash > 0 ? 1.0F : 1.1666666F * (var1.func_72971_b(1.0F) - 0.2F)) * (float)(var5 - 1);
					...
					MCPatcherUtils.getMinecraft().renderEngine.createTextureFromBytes(var8, 16, 16, var0.lightmapTexture);
					return true;
				}
			}
		} else {
	...
	public static int colorizeRedstoneWire(IBlockAccess var0, int var1, int var2, int var3, int var4) {
		if (redstoneColor == null) {
			...
			return float3ToInt(redstoneColor[var5]);
		}
	}
	...
	private static void checkUpdate() {
		if (lastTexturePack != MCPatcherUtils.getMinecraft().texturePackList.getSelectedTexturePack()) {
			lastTexturePack = (TexturePackImplementation) MCPatcherUtils.getMinecraft().texturePackList.getSelectedTexturePack();
			reset();
	...
	private static void reloadXPOrbColors() {
		xpOrbColors = MCPatcherUtils.getImageRGB(MCPatcherUtils.readImage(lastTexturePack.getResourceAsStream("/misc/xporbcolor.png")));
	}
